...
```bash
aws lambda create-function --function-name micronaut \
...
```bash
aws lambda invoke --function-name micronaut --payload '{"resource": "/{proxy+}", "path": "/ping", "httpMethod": "GET"}' build/response.txt
